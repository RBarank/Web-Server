...
namespace http{
  namespace server{
    ...
    request_handler::Status status_handler::HandleRequest(const Request& request, Response* response){
    ...
    request_handler::Status status_handler::Init(const std::string& uri_prefix, const NginxConfig& config)
    {
        return request_handler::OK;
    }
...
std::string setBody()
{
    ...
    */
    ...
    ret += "STATUS\n\n";
    ret += "Number of requests received: " + std::to_string(http::server::ServerInfo::getInstance().get_number_of_requests());
    ret += "\n";
    
    std::vector<http::server::HandlerInfo> handler_info = http::server::ServerInfo::getInstance().ret_handler_info();
    std::vector<http::server::RequestInfo> request_info = http::server::ServerInfo::getInstance().ret_request_info();
